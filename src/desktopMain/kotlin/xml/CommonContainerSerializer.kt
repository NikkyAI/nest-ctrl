...
interface Container<ELEMENT: Element> {
...
abstract class CommonContainerSerializer<CONTAINER: Container<ELEMENT>, ELEMENT: Element> : XmlSerializer<CONTAINER> {
    /** We need to have the serializer for the elements */
    abstract val elementSerializer: KSerializer<ELEMENT> //  = serializer<ELEMENT>()
...
//    }
    ...
    override fun serialize(encoder: Encoder, value: CONTAINER) {
        encoder.encodeStructure(descriptor) {
            encodeSerializableElement(descriptor, 0, ListSerializer(elementSerializer), value.elements)
        }
    }
    ...
    override fun deserializeXML(
            ...
            while (input.next() != EventType.END_ELEMENT) {
                when (input.eventType) {
                    ...
                    EventType.START_ELEMENT -> {
                        if (input.namespaceURI.isEmpty() && input.localName.startsWith(prefix)) {
                            // When reading the child tag we use the DynamicTagReader to present normalized XML to the
                            // deserializer for elements
                            val filter = DynamicTagReader(input, elementXmlDescriptor, prefix = prefix)
        ...
        return constructContainer(dataList)
    }
    ...
    override fun serializeXML(encoder: Encoder, output: XmlWriter, value: CONTAINER, isValueChild: Boolean) {
        ...
        encoder.encodeStructure(descriptor) { // create the structure (will write the tags of Container)
            for (element in value.elements) { // write each element
                ...
                val writer = DynamicTagWriter(output, elementXmlDescriptor, element.index.toString(), prefix = prefix)
